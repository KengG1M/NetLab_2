...
func listenForBroadcasts() {
	addr, _ := net.ResolveUDPAddr("udp", ":404") // Use a different port
...
func startFileServer() {
	ln, err := net.Listen("tcp", ":8081") // Different TCP port
	if err != nil {
...
func downloadFile(fileInfo FileInfo) {
	conn, err := net.Dial("tcp", fileInfo.Owner+":8080") // Connect to Peer 1's TCP
	if err != nil {
...
func listenForSearch() {
	addr, _ := net.ResolveUDPAddr("udp", ":9002") // Different listen port
	conn, err := net.ListenUDP("udp", addr)
	if err != nil {
